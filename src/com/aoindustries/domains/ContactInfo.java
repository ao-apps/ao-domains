...
public class ContactInfo implements Cloneable {
    ...
    public static final int
        ...
        EMAIL_MAX_LENGTH = 80,
        STREET_ADDRESS_1_MAX_LENGTH = 30,
        STREET_ADDRESS_2_MAX_LENGTH = 30,
        CITY_MAX_LENGTH = 30,
        STATE_MAX_LENGTH = 30,
    ...
    public Map<String,List<String>> validate(Locale userLocale) {
        ...
        if(firstName!=null && firstName.length()>FIRST_NAME_MAX_LENGTH) addError(userLocale, errors, "firstName", "ContactInfo.validate.firstName.tooLong", FIRST_NAME_MAX_LENGTH);
        if(lastName!=null && lastName.length()>LAST_NAME_MAX_LENGTH) addError(userLocale, errors, "lastName", "ContactInfo.validate.lastName.tooLong", LAST_NAME_MAX_LENGTH);
        if(companyName==null) {
        ...
        if(streetAddress1==null) addError(userLocale, errors, "streetAddress1", "ContactInfo.validate.streetAddress1.required");
            ...
            if(!streetAddress1.matches("^[a-zA-Z0-9 #&'()+,-./:;@[\\]]+$")) addError(userLocale, errors, "streetAddress1", "ContactInfo.validate.streetAddress1.invalid");
        }
        if(streetAddress2!=null) {
            ...
            if(!streetAddress2.matches("^[a-zA-Z0-9 #&'()+,-./:;@[\\]]+$")) addError(userLocale, errors, "streetAddress2", "ContactInfo.validate.streetAddress2.invalid");
        }
        if(city==null) addError(userLocale, errors, "city", "ContactInfo.validate.city.required");
        else if(city.length()>CITY_MAX_LENGTH) addError(userLocale, errors, "city", "ContactInfo.validate.city.tooLong", CITY_MAX_LENGTH);
        if(state==null) {
        ...
        if(postalCode==null) addError(userLocale, errors, "postalCode", "ContactInfo.validate.postalCode.required");
            ...
            if(!postalCode.matches("^[a-zA-Z0-9 #&'()+,-./:;@[\\]]+$")) addError(userLocale, errors, "postalCode", "ContactInfo.validate.postalCode.invalid");
        }
        ...
        else if(!phone.matches("^+[0-9]{1,3}\\.[0-9]{1,12}$")) addError(userLocale, errors, "phone", "ContactInfo.validate.phone.invalid");
        if(fax!=null && !fax.matches("^+[0-9]{1,3}\\.[0-9]{1,12}$")) addError(userLocale, errors, "fax", "ContactInfo.validate.fax.invalid");
        return errors;
    }
}
