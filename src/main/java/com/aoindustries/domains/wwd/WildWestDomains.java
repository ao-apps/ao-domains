...
public class WildWestDomains implements DomainRegistrar {
  ...
  public Map<Domain, Boolean> checkAvailability(Set<Domain> domains) throws IOException {
    ...
    try {
      ...
      for (Domain domain : domains) if (!results.containsKey(domain)) {
        throw new IOException("TODO: Domain not in results: "+domain);
      }
  ...
  private void resetCertification() throws IOException {
    ...
    logger.fine(response);
    // Parse response
    if (!"scripting status reset".equals(response)) {
      throw new IOException("TODO: "+response);
    }
  }
  ...
  public Map<String, List<String>> poll() throws IOException {
    ...
    try {
      ...
      for (int c=0; c<nodeList.getLength(); c++) {
        ...
        if (resources == null) {
          results.put(orderid, resources = new ArrayList<>());
        }
  ...
  public void certify() throws IOException {
    ...
    if (!Boolean.TRUE.equals(availability.get(exampleUs))) {
      throw new IOException("TODO: example.us is not available");
    }
    if (!Boolean.TRUE.equals(availability.get(exampleBiz))) {
      throw new IOException("TODO: example.biz is not available");
    }
    // order domains
    ...
    if (!Boolean.FALSE.equals(availability2.get(exampleUs))) {
      throw new IOException("TODO: example.us is available");
    }
    if (!Boolean.FALSE.equals(availability2.get(exampleBiz))) {
      throw new IOException("TODO: example.biz is available");
    }
    // information query
    info(bizResourceid);
    // renewal
